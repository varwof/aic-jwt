...
**Document name**: draft-wei-aic-jwt-00
...
`draft-wei-aic-identity-cert-00`.  The AIC X.509 extension binds an AI
...
The AIC X.509 extension defined in `draft-wei-aic-identity-cert-00`
...
| Signature framework | X.509 / RFC 5280 | JWS / RFC 7515 |
...
This document uses the terms defined in `draft-wei-aic-identity-cert-00`
...
3. **Online-first, OAuth-native verification.**  Verification follows
   ...
   cached), token status from Token Status Lists, and sender binding
...
5. **OAuth alignment.**  Standard JOSE and OAuth primitives are reused
   ...
   (RFC 9396), token exchange (RFC 8693), JWT assertions (RFC 7523),
   and Token Status Lists (draft-ietf-oauth-status-list).
...
* **Principal**: the natural person or organization that signs the DA
  ...
  `aic.principal.key_hash`.
* **Agent**: the AI agent presenting the token.  The agent identity is
  the `sub` claim.  The agent key is bound by `cnf`.
...
The Agent acts as the OAuth **actor** (RFC 8693 `actor_token`); the
Principal is the **subject** (`subject_token`).  The AIC-JWT can serve
as an access token (RFC 9068-style), an assertion (RFC 7523), or the
output of a token exchange (RFC 8693).
...
1. The principal creates the DA JWT (Section 5.3) and signs it with the
...
* `iss` (REQUIRED): the issuer identifier, a URL per [RFC9068] and
  [RFC9207].
* `sub` (REQUIRED): the `agentId`, a string of 1 to 256 characters.
  ...
  from RFC 9068's default `sub` semantics (the OAuth resource owner);
  the responsible principal is carried in `aic.principal`.
...
* `status` (OPTIONAL): a Token Status List reference per
  `draft-ietf-oauth-status-list`, with `idx` and `uri` members.
...
* `principal` (REQUIRED): the principal binding, with members:
  ...
  * `key_hash` (REQUIRED): the principal binding hash, either
    (a) the base64url encoding of `hash_alg(SPKI)` where SPKI is the
    DER SubjectPublicKeyInfo of the principal's X.509 certificate, or
    (b) the RFC 7638 JWK thumbprint (`jkt`) of the principal's JWK in
...
The DA JWT payload is the JSON equivalent of DelegationAuthTBS.  All
ten members MUST be present:
...
* `ver` (REQUIRED): 1.
* `agent_id` (REQUIRED): the agent identifier; MUST equal the outer
  `sub`.
...
log records `aic.principal.id` as the actor and `sub` as the executor.
The bundle MUST contain the principal's PA material.  At issuance and
at runtime, `C_agent` MUST be a subset of `P_grants`.  Wide scope x
short lifetime, with runtime `P_grants` intersection at each
operation.
...
5. The CA constructs and signs the outer AIC-JWT, with
   `exp - iat = min(requested_lifetime, local policy cap)`.
...
2. The Agent presents the principal-signed DA JWT as an assertion,
   using the RFC 7523 `grant_type`
   (`urn:ietf:params:oauth:grant-type:jwt-bearer`) or as a
   `client_assertion`/`client_assertion_type` pair, together with the
   token request.
3. The AS validates the DA JWT (signature, nonce uniqueness, subset
   checks) and issues the outer AIC-JWT signed with the AS key.
4. The AS MAY expose a JWKS endpoint and Token Status List for
...
An AIC-JWT MAY be produced or consumed through RFC 8693 token
exchange:
...
revocation uses Token Status Lists.  No ASN.1/DER or X.509 processing
...
`draft-wei-aic-identity-cert-00` profile with mTLS is used; the JSON
profile is not.
...
* key binding: `aic.principal.key_hash` with a SHA-2 `hash_alg`
  (SPKI hash) and the RFC 7638 JWK thumbprint of the same key are two
...
* DelegationAuthorization: the ASN.1 DelegationAuthTBS and the DA JWT
  ...
  translate between the two for issuance, verification, or audit;
...
`draft-wei-aic-identity-cert-00`.
...
4. **DA validation** (full profile): verify the inner DA JWT signature
   ...
   check that the DA `nonce` decodes to 32 bytes.  The DA nonce
   ...
   the DA nonce.
5. **Consistency checks**: DA `agent_id == sub`, DA `principal ==
   aic.principal`, DA `capabilities == aic.capabilities`, DA
   `delegation_mode == aic.delegation_mode`, DA `constraints ==
   aic.constraints`.  Any mismatch MUST cause rejection.
...
10. **Status check** (optional): if a `status` claim is present, fetch
    or use a cached Token Status List and verify the referenced token
    is valid.
...
replay.  Where mTLS is deployed, the verifier MAY cross-check the mTLS
...
1. `aic.principal.id` MUST NOT contain raw PII (full names, national
   ...
   deletable database.
2. Revocation (Token Status List or short lifetime) is the mechanism
...
6. No sensitive data MUST be placed in any claim; all AIC-JWT data is
...
| `aic+da` (inner payload members) | DelegationAuthTBS equivalent |
...
8693 token exchange and RFC 7523 assertions.
...
| `sub` = agentId | `sub` = `spiffe://<td>/agent/<id>`, `agent_id` | `sub` (SPIFFE); `sub` (RFC 9068) |
| `aud` | `aud` | `aud` |
